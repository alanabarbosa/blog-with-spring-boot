...
import io.github.alanabarbosa.data.vo.v1.CommentResponseBasicVO2;
...
public class HateoasUtils {
    ...
    public static void addLink(CommentResponseBasicVO2 entity, Long id, String rel) {
        try {
            entity.add(linkTo(methodOn(CommentController.class).findById(id)).withRel(rel));
        } catch (Exception e) {
            Logger.getLogger(HateoasUtils.class.getName())
    ...
    public static void addCommentLinksResponse2(List<CommentResponseBasicVO2> comments) {
        comments.forEach(comment -> 
            addLink(comment, comment.getKey(), "comments-details"));
    }
}
